...
| A-4 | Global settings are stored in a `Settings.yaml` file (YAML, `YamlDotNet`) loaded as a singleton. Configurable fields: `SqlConnection`, `ReportTemplate`, `Theme` (Dark/Light), `Culture`, `Extension`, `VirtualDirectories`, `Preview`, `Backend`, `Version`, `Modules`, `UpdateUrl`. |
...
| V-9  | A **Generate Recall** dialog (`GenerateRecallDialog`) exists (purpose: generate a recall/report document for the label).                                                                                                                                                             |
